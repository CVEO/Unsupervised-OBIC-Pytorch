...
#####################this is part of loop###############    
    images_path="{}/".format(args.input)
    images_paths=[p.split('.')[0] for p in os.listdir(images_path)]
    _images_paths=[p for p in images_paths if p[-3:]!='seg']
    for p in _images_paths:
        input_image_path=images_path+p+'.tif'
        seg_path=images_path+p+'_seg.tif'
...
#################################this is the original code###########
        ...
        for batch_idx in range(args.epochs):
            ...
            for y_offset, x_offset, y_buffersize, x_buffersize, _ in tqdm(shuffled_patches_info):            
                ...
                tensor = torch.unsqueeze(torch.from_numpy(buffer).to(device)/ 255.0, 0) 
            ...
            SaveLabelArrayInCompressMode(im_target, output_path, seive_small_area=True)#保存tiff
